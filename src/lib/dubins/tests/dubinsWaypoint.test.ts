...
test("Split Dubins runs no runs", () => {
  const a: Command[] = []
  a.push(makeCommand("MAV_CMD_NAV_WAYPOINT", { latitude: 0, longitude: 0 }))
  a.push(makeCommand("MAV_CMD_NAV_WAYPOINT", { latitude: 0, longitude: 0 }))
  a.push(makeCommand("MAV_CMD_NAV_WAYPOINT", { latitude: 0, longitude: 0 }))
  let runs = splitDubinsRuns(a)
  expect(runs.length).toBe(0)
...
test("Split Dubins runs sandwich 1", () => {
  const a: Command[] = []
  a.push(makeCommand("MAV_CMD_NAV_WAYPOINT", { latitude: 0, longitude: 0 }))
  a.push(makeCommand("WM_CMD_NAV_DUBINS", { latitude: 0, longitude: 0 }))
  a.push(makeCommand("MAV_CMD_NAV_WAYPOINT", { latitude: 0, longitude: 0 }))
  a[1].frame = 0
  let runs = splitDubinsRuns(a)
  ...
  expect(runs[0].wps[1].frame).toBe(0)
})
...
test("Split Dubins runs start + end", () => {
  ...
  expect(runs[0].wps.length).toBe(2)
  expect(runs[0].wps[0].frame).toBe(0)
  ...
  expect(runs[1].wps[1].frame).toBe(10)
})
