...
import { bearing, dist, mod2pi, offset, pathLength, world_dist } from "./geometry";
...
export function DubinsBetween(a: XY, b: XY, theta_a: number, theta_b: number, turn_radius: number): Path{
  ...
  if (world_dist(a_centers.r, b_L_CP) > turn_radius * 2){
    ...
    let BL2: Curve = {type: "Curve", center: b_L_CP, radius: turn_radius, start: (theta_a + Math.PI/2) + ((a - theta_a) - Math.PI * 2), theta:((theta_b - a) - Math.PI * 2) % (Math.PI*2)}
  ...
  console.log(sections.map((a)=>(pathLength(a))))
  return sections[0]
