...
export function DubinsBetweenDiffRad(a: XY, b: XY, thetaA: number, thetaB: number, radA: number, radB: number): Path{
  ...
  if (world_dist(a_centers.r, b_centers.r) > Math.abs(radA - radB)){
    ...
    let c1: Curve = {type: "Curve", center: a_centers.r, radius: radA, start: right_start, theta: mod2pi(a - thetaA)}
    let s: Straight = {type: "Straight", start: worldOffset(a_centers.r, radA, a - Math.PI/2), end: worldOffset(b_centers.r, radB, a - Math.PI/2)}
    let c2: Curve = {type: "Curve", center: b_centers.r, radius: radB, start: a - Math.PI/2, theta: mod2pi(thetaB - a)}
  ...
  if (world_dist(a_centers.l, b_centers.l) > Math.abs(radA - radB)){
    ...
    let c1: Curve = {type: "Curve", center: a_centers.l, radius: radA, start: left_start, theta: mod2pi(a - thetaA) - Math.PI*2}
    let s: Straight = {type: "Straight", start: worldOffset(a_centers.l, radA, a + Math.PI/2), end: worldOffset(b_centers.l, radB, a + Math.PI/2)}
    let c2: Curve = {type: "Curve", center: b_centers.l, radius: radB, start: a - 3 * (Math.PI/2), theta: mod2pi(thetaB - a) - Math.PI*2}
    let RSL: Path = [c1, s, c2]
    sections.push(RSL)
  }
