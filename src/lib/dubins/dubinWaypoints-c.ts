...
export function getTunableDubinsParameters(wps: dubinsPoint[]): number[] {
  ...
  for (let i = 0; i < wps.length; i++) {
    if (wps[i].tunable) {
      ret.push(wps[i].radius)
      ret.push(wps[i].heading)
    }
  }
...
export function setTunableDubinsParameter(wps: dubinsPoint[], params: number[]): void {
  ...
  for (let i = 0; i < wps.length; i++) {
    if (wps[i].tunable) {
      wps[i].radius = params[a]
      a++
      wps[i].heading = params[a]
      a++
    }
  }
...
export function getBounds(wps: dubinsPoint[], vehicle: Plane): bound[] {
  ...
  for (let i = 0; i < wps.length; i++) {
    if (wps[i].tunable) {
      bounds.push({
        min: Math.pow(vehicle.cruiseAirspeed, 2) / (9.8 * Math.tan(deg2rad(vehicle.maxBank)))
      })
      bounds.push({
        min: 0,
        max: 360,
...
export function waypointToDubins(cmd: LatLngCommand, reference: LatLng): dubinsPoint {
  if (cmd.type == 72) {
    return { pos: g2l(reference, getLatLng(cmd)), radius: cmd.params.radius, heading: cmd.params.heading, tunable: true, passbyRadius: cmd.params["fly-by distance"] }
  } else {
    return { pos: g2l(reference, getLatLng(cmd)), radius: 0, heading: 0, tunable: false, passbyRadius: 0 }
  }
}
