...
export const particleOptimise: optimisationAlgorithm = (initialGuess, bounds, fn) => {
  ...
  // weights controlling bias towards personal best, global best, and local best
  const cogWeight = 0.5
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    for (let p = 0; p < popSize; p++) {
      ...
      for (let a = 0; a < initialGuess.length; a++) {
        ...
        const newVel = friction * velocities[p][a] +
        ...
        // randomly flip a circular bound to opposite direction
        if (curBound.circular && curBound.max !== undefined && curBound.min !== undefined && Math.random() > 0.99) {
